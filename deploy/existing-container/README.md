...
1. Install `requirements.lock` into the release's virtual environment, then install the package with `pip install --no-deps <release-directory>`. Use the environment's trusted package mirror if necessary.
